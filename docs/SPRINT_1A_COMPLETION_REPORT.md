...
3. **Intent Parsing Mapper (`lib/lineMessageMapper.ts`)**: State-aware parsing supporting SKU matches, variant questions/resolutions, and simulated payment slip/address inputs.
4. **In-Memory server state store (`lib/serverSimulationStore.ts`)**: Thread-safe hot-reload-guarded global state singleton to hold simulation inputs, orders, items, events, and notifications.
