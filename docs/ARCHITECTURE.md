...
* **Role**: A deterministic finite state machine handling the order flow:
...
* **Role**: Matches incoming slips against bank QR transactions.
* **Rules**: If a duplicate reference is detected, status shifts to `duplicate_slip` and shifts the order to `issue`. If the amount is lower than expected, status shifts to `amount_mismatch` and triggers an `issue` case.

### 2.5 Live Dashboard & Notification Center
* **Role**: Aggregates sales statistics and dispatches alerts (e.g., low stock alerts, mismatch notifications).
* **Modes**: Supports mode-based filtering (`off`, `important_only`, `all`, `live_sale_mode`) to reduce alert fatigue.
